...
func TestOverlapsLoopBreak(t *testing.T) {
	var count = 0
	for range testDriver.NewSpanOverlapAccumulator().NewOverlappingSpanSetsIterSeq2FromSpanBoundrySlice(&MultiSet) {
		count++
		break
	}
...
func TestMultSetDataOverlaps(t *testing.T) {
	...
	var count = 0
	for idx, ol := range ac.NewOverlappingSpanSetsIterSeq2FromSpanBoundrySlice(&MultMultiiSet) {
		count++
...
func TestPull2MultSetDataOverlaps(t *testing.T) {
	var next, stop = iter.Pull2(testDriver.NewSpanOverlapAccumulator().NewOverlappingSpanSetsIterSeq2FromSpanBoundrySlice(&MultMultiiSet))
	defer stop()
	var count = 0
	var expected = [][]int{
		{0, 0},
...
func TestOverlapAdjacentConsolidate(t *testing.T) {
	...
	var count = -1
	for idx, ol := range ac.NewOverlappingSpanSetsIterSeq2FromSpanBoundrySlice(&[]SpanBoundry[int]{
